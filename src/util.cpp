...
const char * const BITCOIN_CONF_FILENAME = "momo.conf";
const char * const BITCOIN_PID_FILENAME = "momod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "momo" is a composite category enabling all Momo-related debug output
            if(ptrCategory->count(string("momo"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MOMOCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MOMOCore
    // Mac: ~/Library/Application Support/MOMOCore
    // Unix: ~/.momocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MOMOCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MOMOCore";
#else
    // Unix
    return pathRet / ".momocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty momo.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override momo.conf
