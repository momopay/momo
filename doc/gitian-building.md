...
*Setup instructions for a Gitian build of Momo Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Momo
...
- [Building Momo Core](#building-momo-core)
...
Clone the git repositories for Momo Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/momopay/momo
```
...
in the Momo Core repository under 'Fetch and build inputs' to install sources which require
...
To build Momo Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Momo Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/momo/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/momopay/momo
...
For example:
```bash
URL=https://github.com/crowning-/momo.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit momo=${COMMIT} --url momo=${URL} ../momo/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit momo=${COMMIT} --url momo=${URL} ../momo/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit momo=${COMMIT} --url momo=${URL} ../momo/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the momo git repository with the momod tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../momo/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/momopay/momo-detached-sigs.git

BTCPATH=/some/root/path/momo.git
SIGPATH=/some/root/path/momo-detached-sigs.git

./bin/gbuild --url momo=${BTCPATH},signature=${SIGPATH} ../momo/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/momo-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/momo-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/momo-osx-build.assert
```
...
[momo/gitian.sigs](https://github.com/momopay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
